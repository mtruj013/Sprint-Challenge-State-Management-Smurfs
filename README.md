...
- [ ] What problem does the context API help solve?
  ...
  Passing props through many different components can become cumbersome, but context API provides a way to share values like props between components without having to explicitly pass a prop through every level of the tree. 


- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

  actions: payloads of information that send data from your application to your store,
  store: object that allows components to share state, like a database,
  reducers: functions that take in two arguements (state and action) and return a new updated state

- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?

  Application state is global, component state is local. A form would be a good candidate for component state, and a list of cards would be a good candidate for application state

- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

  It's a type of middleware that allows us to intercept every action that is dispatched before it gets to the reducer.The thunk can be used to delay the dispatch of an action, or to dispatch only if a certain condition is met

- [ ] What is your favorite state management system you've learned and this sprint? Please explain why!

  Context Api, it's easier to track the data 
